...
class TrackSelectorViewController: UICollectionViewController, PlaybackEngineDelegate {
    ...
    func updateTime(currentTime: AVMusicTimeStamp) {
        //        print(CGFloat(currentTime / sequenceLength!)
        guard PlaybackEngine.shared.isPlaying else {
            return
        }
        DispatchQueue.main.async {
            self.selectedCell.overlay.frame.size.width = CGFloat(currentTime / self.sequenceLength!) * self.selectedCell.frame.size.width
        }
    ...
    func didFinishPlaying() {
        //
        DispatchQueue.main.async {
            self.selectedCell.overlay.frame.size.width = 0
        }
    }
    ...
    func getDataFromBlock() {
        tracks = block.parsedTracks.map({ (track) -> ScatterChartData in
            let data = track.notes.map{ChartDataEntry.init(x: $0.timeStamp, y: Double($0.note))}
...
class TrackTile: UICollectionViewCell {
    ...
    func setTile(data: ScatterChartData, name: String) {
        let chart = ChartView(frame: self.bounds)
        chart.data = data
...
//        print((noteInCurrentBlock_sorted.map{($0.onTrack, $0.noteEvent.timeStamp)}), (noteInSecondBlock_sorted.map{($0.onTrack, $0.noteEvent.timeStamp)}))
    ...
    func makeDecision(input:([NoteOnTrack], [NoteOnTrack]),chanceOfMutation: Double) -> [NoteOnTrack] {
        ...
        if (0 ..< chanceOfMutation * 100).contains(dice) {
            var notes = input.1
            for i in notes {
                let mappedTrack = trackMapping(input: i)
                if mappedTrack != nil {
                    i.onTrack = mappedTrack!
